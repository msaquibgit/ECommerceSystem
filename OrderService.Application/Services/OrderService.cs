...
namespace OrderService.Application.Services
{
    public class OrderService : IOrderService
    {
        ...
        )
        {
            ...
            _masterDataRepository = masetDataRepository ?? throw new ArgumentNullException(nameof(masetDataRepository));
        ...
        public async Task<OrderResponseDTO> CreateOrderAsync1(CreateOrderRequestDTO request, string accessToken)
        {
            if (request == null) throw new ArgumentNullException(nameof(request));
            if (request.Items == null || !request.Items.Any()) throw new ArgumentException("Order must have at least one item.");
            bool userExist = await _userServiceClient.UserExistsAsync(request.UserId, accessToken);
            if (!userExist) throw new InvalidOperationException("User does not exist");
            ...
            try
            {
                ...
                var cancellationPolicy = _masterDataRepository.GetActiveCancellationPolicyAsync();
                ...
                if (paymentResponse == null) throw new InvalidOperationException("Payment initiation failed.");
                // For COD, immediately reserve stock and send notification
                if (request.PaymentMethod == PaymentMethodEnum.COD)
                {
                    var stockUpdate = request.Items.Select(i => new UpdateStockRequestDTO
                    {
                        ProductId = i.ProductId,
                        Quantity = i.Quantity

                    }).ToList();
                    var stockUpdated = await _productServiceClient.DecreaseStockBulkAsync(stockUpdate, accessToken);
                    if (!stockUpdated) throw new InvalidOperationException("Failed to reserve product stock for COD order.");
        ...
        public async Task<bool> ConfirmOrderAsync1(Guid orderId, string accessToken)
        {
            // Retrieve order
            ...
            try
            {
                ...
                var updateStock = order.OrderItems.Select(i => new UpdateStockRequestDTO
                {
                    ProductId = i.ProductId,
                    Quantity = i.Quantity,
                }).ToList();
                bool stockReduced = await _productServiceClient.DecreaseStockBulkAsync(updateStock, accessToken);
                if (!stockReduced) throw new InvalidOperationException("Failed to reduce stock after payment.");
                // Notify user asynchronously about successful order placement
                _ = _notificationServiceClient.SendOrderPlacedNotificationAsync(order.UserId, orderId, accessToken);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<ChangeOrderStatusResponseDTO> ChangeOrderStatusAsync(ChangeOrderStatusRequestDTO request)
        {
            ...
            try
            {
                ...
                bool statusChanged = await _orderRepository.ChangeOrderStatusAsync(request.OrderId, request.NewStatus, request.ChangedBy ?? "System", request.Remarks);
                if (!statusChanged)
                {
                ...
                _ = _notificationServiceClient.SendOrderStatusChangedNotificationAsync(order.UserId, request.OrderId, "");
        ...
        public async Task<PaginatedResultDTO<OrderResponseDTO>> GetOrdersByUserAsync(Guid userId, int pageNumber = 1, int pageSize = 20)
        {
            ...
            var totalOrders = orders.Count(); // Consider separate count method for true total
