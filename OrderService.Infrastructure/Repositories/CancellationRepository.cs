...
namespace OrderService.Infrastructure.Repositories
{
    public class CancellationRepository : ICancellationRepository
    {
        ...
        public CancellationRepository(OrderDbContext context)
        {
        ...
        public async Task<Cancellation?> GetByIdAsync(Guid cancellationId)
        {
            ...
            var cancellation = await _context.Cancellations.AsNoTracking()
                .Include(c => c.CancellationItems)
                    .ThenInclude(ci => ci.OrderItemId)
        ...
        public async Task<Cancellation?> UpdateAsync(Cancellation cancellation)
        {
            ...
            var existing = await _context.Cancellations
                    .Include(c => c.CancellationItems)
            ...
            _context.CancellationItems.RemoveRange(itemsToRemove);
            ...
            if (cancellation.CancellationItems != null)
            {
                foreach (var incomingItem in cancellation.CancellationItems)
                {
                    ...
                    else
                    {
                        ...
                        _context.CancellationItems.Add(incomingItem);
                    }
                }
            }
            await _context.SaveChangesAsync();
        ...
        public async Task<List<Guid>> GetCancelledOrderItemIdsAsync(Guid orderId)
        {
            ...
            // and cancellation status is Pending or Approved (assuming those are statuses 1 and 2)
            var cancelledItemIds = await _context.CancellationItems
        ...
        {
            return await _context.CancellationItems
                .AsNoTracking()
                .Where(ci => ci.CancellationId == cancellationId)
                .ToListAsync();
        }
        public async Task<List<CancellationItem>> GetCancellationsByOrderIdAsync(Guid cancellationId)
        {
            return await _context.CancellationItems
                .AsNoTracking()
                .Where(ci => ci.CancellationId == cancellationId)
        ...
        public async Task ApproveAsync(Cancellation updatedCancellation, OrderStatusEnum newOrderStatus, string processedBy, string? remarks = null)
        {
            if (updatedCancellation == null) throw new ArgumentNullException(nameof(updatedCancellation));
