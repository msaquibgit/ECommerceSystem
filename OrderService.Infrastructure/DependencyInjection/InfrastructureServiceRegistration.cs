...
namespace OrderService.Infrastructure.DependencyInjection
{
    public static class InfrastructureServiceRegistration
    {
        public static IServiceCollection AddInfrastructureServices(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            {
                var baseUrl = configuration["ExternalServices:UserServiceUrl"] ?? throw new ArgumentNullException("UserServiceUrl not configured");
                client.BaseAddress = new Uri(baseUrl);
            });

            services.AddHttpClient("ProductServiceClient", client =>
            {
                var baseUrl = configuration["ExternalServices:ProductServiceUrl"] ?? throw new ArgumentNullException("ProductServiceUrl not configured");
                client.BaseAddress = new Uri(baseUrl);
            });

            services.AddHttpClient("PaymentServiceClient", client =>
            {
                var baseUrl = configuration["ExternalServices:PaymentServiceUrl"] ??  throw new ArgumentNullException("PaymentServiceUrl not configured");
                client.BaseAddress = new Uri(baseUrl);
            });
